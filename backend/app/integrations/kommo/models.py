...
from sqlalchemy import String, DateTime, ForeignKey, Text, Integer, UniqueConstraint, Index
...
class CRMIntegration(Base):
    ...
    __table_args__ = (
        UniqueConstraint("organization_id", "subdomain", name="uq_crm_integration_org_subdomain"),
    )
...
class IntegrationLog(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    organization_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("organizations.id", ondelete="CASCADE"), index=True, nullable=True)
