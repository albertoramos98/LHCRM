...
async def generate_dashboard_html(session: AsyncSession, output_path: str = "static/dashboard.html") -> str:
    """
    Compiles all dashboard metrics, renders them into the public HTML template,
    and writes the resulting standalone file to the specified output path.
    """
    try:
        # 1. Fetch metrics from the DashboardService
        ...
        logger.info(f"Public HTML dashboard successfully written to: {output_path}")
        return rendered_content

    except Exception as e:
        logger.error(f"Failed to generate public HTML dashboard: {e}", exc_info=True)
        raise e
